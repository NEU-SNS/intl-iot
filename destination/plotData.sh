...
for f in $(find $expDir -name "*.pcap" | grep -v companion | grep "2019-05-08_"); do
  ...
  deviceList="aux/devices_${country}.txt"
	...
	echo python analyse.py -i $f -s aux/tshark_all.hosts -d $device -c $deviceList -e $experiment -b $lab -n $network -o $output
	python analyse.py -i $f -s aux/tshark_all.hosts -d $device -c $deviceList -e $experiment -b $lab -n $network -o $output
done;
